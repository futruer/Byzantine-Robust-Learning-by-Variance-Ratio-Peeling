...
def _agglomerative_clustering_average_linkage(distances_mat: torch.Tensor, n_clusters: int = 2) -> List[List[int]]:
    """
    修改为文献要求的：带有平均链接 (Average Linkage) 的凝聚层次聚类。
    """
    ...
    while len(clusters) > n_clusters:
        ...
        for i in range(len(clusters)):
            for j in range(i + 1, len(clusters)):
                ...
                # 【核心修改点】：Average Linkage 计算两个簇之间所有成员距离的平均值
                sum_d = 0.0
                count = 0
                for idx_i in cluster_i_idx:
                    for idx_j in cluster_j_idx:
                        sum_d += distances_mat[idx_i, idx_j].item()
                        count += 1
                
                avg_d = sum_d / count  # 取平均距离
...
def clustering_based(gradients_list: List[Dict], n_clusters: int = 2) -> Dict[str, torch.Tensor]:
    ...
    clusters = _agglomerative_clustering_average_linkage(distances, n_clusters=2)
