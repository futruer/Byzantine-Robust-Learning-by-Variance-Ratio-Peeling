...
# CIFAR-10 数据处理拓展
# ==========================================
def get_cifar10_client_dataloaders(
    num_clients: int,
    batch_size: int,
    root: str = './data',
    download: bool = True
    ...
    """
    加载 CIFAR-10 数据集，将其随机均匀分成 num_clients 份
    """
    transform = transforms.Compose([
        transforms.ToTensor(),
    ...
    train_dataset = datasets.CIFAR10(root=root, train=True, download=download, transform=transform)
    total_len = len(train_dataset)
    ...
    print(f"Data process finished: Split CIFAR-10 ({total_len} samples) into {num_clients} clients.")
    return client_data_list

def get_cifar10_proxy_dataloader(
    batch_size: int,
    root: str = './data',
    download: bool = True
) -> DataLoader:
    """
    获取服务器端干净代理数据的 DataLoader (CIFAR-10)
    """
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010))
    ...
    # CIFAR-10 共有 50000 训练集。按 5% 算，取前 2500 个样本作为代理数据
    proxy_size = 2500
    proxy_dataset = Subset(train_dataset, range(proxy_size))
    proxy_loader = DataLoader(proxy_dataset, batch_size=batch_size, shuffle=True)

    print(f"Proxy data loader created (CIFAR-10): {proxy_size} samples")
    return proxy_loader
