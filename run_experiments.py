...
from CNN.cnn_data_process import get_mnist_client_dataloaders, cnn_reform_func
...
def print_header():
    """打印实验头部信息"""
    print("=" * 70)
    print("           Byzantine-Robust Federated Learning Experiments")
    print("=" * 70)
...
def print_experiment_info(agg_name, attack_type, attack_ratio, exp_num, total):
    ...
    # 聚合方法映射
    agg_descriptions = {
        'peeling': 'VR Peeling + FedAvg (你的方法：先筛选，后均值聚合)',
...
def get_attack_func(attack_type):
    """获取攻击函数"""
    attacks = {
        'label_flipping': None,  # 在数据层面进行，不需要梯度攻击
...
def get_aggregation_func(agg_name):
    ...
    aggs = {
        'peeling': partial(aggregation.coordinate_wise_trimmed_mean, beta=config.TRIMMED_MEAN_BETA),        
...
def run_benchmark_training(model, clients, epochs, learning_rate, aggregation_func, gradient_attack_func, device, verbose=True):
    ...
    for epoch in range(epochs):
        ...
        total_samples = 0
        with torch.no_grad():
            # 仅在正常数据集上评估全局模型表现 (为了公平，评估指标应不含毒数据)
            for client in clients:
...
def run_experiment(agg_name, attack_type, attack_ratio, epochs, device='cpu', seed=42, verbose=True):
    ...
    if verbose:
        print(f"  [2/5] 设置攻击: {attack_type} (比例: {attack_ratio*100:.0f}%)")
    ...
    # ==========================================
    # 核心分流：你的方法 vs Benchmark
    # ==========================================
    use_screening = (agg_name == 'peeling')  

    if verbose:
        screening_info = "启用筛选（你的方法 Orchestrator）" if use_screening else "不筛选（Benchmark 全部收录）"
        print(f"  [4/5] 训练模式: {screening_info}")

    if use_screening:
        # 【你的方法】：先通过 Orchestrator 筛选坏节点，再用 agg_func(当前设为FedAvg) 聚合好节点
        ...
        # 【Benchmark 方法】：完全不通过 Orchestrator，直接接收所有节点数据进行鲁棒聚合
    ...
    if verbose:
        print(f"  [5/5] 训练完成! 测试准确率: {accuracy:.2f}%")
        print(f"       耗时: {elapsed_time:.2f}秒")

    return accuracy, elapsed_time


def run_experiments(output_file='results.csv'):
    device = config.DEVICE
    print_header()

    # 加入了 fedavg 作为无防御的垫底基准，突出展示攻击的破坏性
    agg_methods = ['peeling', 'fedavg', 'trimmed_mean', 'multi_krum', 'clustering']
...
def main():
    ...
    parser.add_argument('--output', type=str, default='results.csv', help='输出 CSV 文件路径')
